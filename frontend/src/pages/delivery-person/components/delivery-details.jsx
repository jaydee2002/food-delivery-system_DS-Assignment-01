...
import { getDeliveries, getDeliveryById, getOrderDetails, updateDeliveryStatus } from "../api/delivery.js";
...
function DeliveryDetails() {
  ...
  useEffect(() => {
    const fetchDeliveryAndOrder = async () => {
      try {
        const deliveries = await getDeliveryById(deliveryId);
        setDelivery(deliveries);

// Fetch order details
        const orderData = await getOrderDetails(deliveries.order);
        setOrder(orderData);
  ...
  <div className="container mx-auto p-4 md:p-6 max-w-4xl">
    <div className="flex items-center justify-between mb-6">
      ...
      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
        ...
        {delivery.status.replace(/-/g, ' ').toUpperCase()}
      </span>
    </div>
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                ...
                <p className="text-sm">
                  ...
                  <span className="ml-2 text-gray-600">{delivery.location}</span>
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                <p className="text-sm">
                  ...
                  <span className="ml-2 font-mono text-indigo-600">{order.customer._id}</span>
                </p>
              </div>
            </div>
          </div>
                ...
                {order.items.map((item) => (
                  <tr key={item._id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      <span className="font-mono text-indigo-600 mr-2">{item.menuItem._id}</span>
                    </td>
