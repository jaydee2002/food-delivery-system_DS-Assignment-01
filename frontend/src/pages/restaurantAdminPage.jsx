...
const RestaurantAdminPage = ({ restaurantId }) => {
 const [orderGroups, setOrderGroups] = useState({
  ...
  const renderOrder = (order) => (
    <li key={order._id} className="bg-white rounded-xl shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg">
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h3 className="text-lg font-bold text-gray-800">Order #{order._id.slice(-6).toUpperCase()}</h3>
            <p className="text-sm text-gray-500">{new Date(order.createdAt).toLocaleString()}</p>
          </div>
          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
            order.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
            order.status === 'preparing' ? 'bg-blue-100 text-blue-800' :
            order.status === 'ready-to-pickup' ? 'bg-purple-100 text-purple-800' :
            order.status === 'picked' ? 'bg-indigo-100 text-indigo-800' :
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="bg-gray-50 p-3 rounded-lg">
            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Restaurant</h4>
            <p className="font-medium text-gray-800">{order.restaurant?.name || "Unknown"}</p>
          </div>
        </div>
          ...
          <ul className="space-y-2">
            {order.items.map((item, index) => (
              <li key={index} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0">
                <div>
                  <p className="font-medium text-gray-800">{item.menuItem?.name || "Unknown Item"}</p>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-gray-200">
          <p className="font-bold text-lg">Total: <span className="text-indigo-600">${order.total.toFixed(2)}</span></p>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="space-y-10">
          {[
            { title: "Pending Orders", status: "pending", orders: orderGroups.pending },
            { title: "Preparing Orders", status: "preparing", orders: orderGroups.preparing },
            { title: "Ready for Pickup", status: "ready", orders: orderGroups.ready },
            { title: "Picked Orders", status: "picked", orders: orderGroups.picked },
            { title: "Delivered Orders", status: "delivered", orders: orderGroups.delivered }
          ].map((group) => (
            <div key={group.status} className="">
              ...
              {group.orders.length > 0 ? (
                ...
                <div className="bg-white rounded-xl shadow-sm p-8 text-center">
                  ...
                  <h3 className="mt-2 text-lg font-medium text-gray-900">No {group.title.toLowerCase()}</h3>
                  <p className="mt-1 text-gray-500">All caught up! New orders will appear here.</p>
                </div>
              )}
            </div>
          ))}
